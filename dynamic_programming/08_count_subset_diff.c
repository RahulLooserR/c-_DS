 ...
 * Last modified : Thursday 05 November 2020 07:57:47 AM IST
 * Description   : 
 * ***********************************************************************/
...
int count_subset_with_given_diff(int diff, int *arr, int n)
{
	/* s2 - s1 = diff
	 * s2 + s1 = sum
	 * 2s2 = sum + diff
	 ...
	 */
	return count_subset((sum(arr, n) + diff) / 2, arr, n);
}
...
int main()
{
	...
	printf("Number of subsets with difference %d : %d\n", diff, count_subset_with_given_diff(diff, arr, n));
